...
procedure Main is
   ...
   function To_Row (U : User) return Database.Rows.Row is
      R : Database.Rows.Row;
   begin
...
begin
   if Ada.Directories.Exists ("example_persistent.database") then
      Ada.Directories.Delete_File ("example_persistent.database");
   end if;
   ...
   R := Users.Insert (Tx, DB, S, (1, "Ada             ", 42));
   pragma Assert (Database.Status.Is_Ok (R), "insert failed");
